"""
feature_pipeline_fixed.py
==========================
Production-grade, leakage-free feature engineering pipeline for BTC/USD.
Combines hourly OHLCV market data with hourly Reddit sentiment.

TIME ALIGNMENT CONTRACT
───────────────────────
  • Every candle at timestamp T represents the interval [T-1h, T).
    Convention: Pandas default resample label is the LEFT (start) edge of the
    bucket.  resample_ohlcv must be called with label='left', closed='left'
    (confirmed via OHLCV_RESAMPLE_CONVENTION below).

  • Sentiment rows carry a time_window timestamp that marks the START of the
    hourly window during which posts/comments were created, i.e. activity in
    [time_window, time_window+1h).

  • After the merge at timestamp T, a model sees:
      – Market candle that CLOSED at T  (interval [T-1h, T))
      – Sentiment published in [T-1h, T)  ← shifted by 1h before merge
    This guarantees zero forward leakage.

LEAKAGE FIXES SUMMARY  (detailed inline)
─────────────────────
  FIX-1  : OHLCV resampling – enforce label='left', closed='left' and shift
            resampled timestamp +1h so the label equals bar-close time.
  FIX-2  : Sentiment time-alignment – shift time_window +1h before merge so
            activity in window [T-1h,T) maps to market bar closing at T.
  FIX-3  : Forward-fill ordering – ffill is applied AFTER the +1h shift, so
            we only propagate past (not future) values.
  FIX-4  : EMA / rolling on sentiment – min_periods enforced; no look-ahead
            in ewm because adjust=False is strictly causal.
  FIX-5  : Volatility rolling std – uses min_periods=1 but the window is
            strictly backward-looking (.rolling without center=True).
  FIX-6  : Divergence scale mismatch – normalize sentiment_combined to the
            same scale as returns before computing divergence.
  FIX-7  : Sentiment spike – rolling mean uses shift(1) so current bar's own
            volume does not contribute to the threshold for that same bar.
  FIX-8  : Target construction – target and target_return use shift(-1) of
            a column that contains NO future data (close is backward-looking).
  FIX-9  : Scaler note – scalers MUST be fit on train split only; guard added
            via ScalerGuard helper and documented assertion.
  FIX-10 : price_momentum replaced with normalized version to remove scale
            mismatch with sentiment features.
"""
