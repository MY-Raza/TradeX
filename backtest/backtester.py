...
class BacktestConfig:
    ...
    leverage: float = 1.0
    transaction_fee: float = 0.05   # percent
    slippage: float = 0.0           # percent
    take_profit_pct: float = 0.03   # 3% = 0.03
    stop_loss_pct: float = 0.01     # 1% = 0.01
...
class Backtester:
    ...
    def run(self, price_df: pd.DataFrame, signal_df: pd.DataFrame):
        ...
        for i, (start_idx, end_idx) in enumerate(intervals):
            ...
            # Inherit direction for zero signals
            ...
            if in_position and current_direction != prev_direction:
                ...
                # Immediate re-entry in new direction
                buy_price = opens[entry_idx]
                direction = current_direction
            ...
            if in_position:
                ...
                if hit_idx is not None:
                    # 🔥 Use actual candle wick price (matches backtest1)
                    if direction > 0:
                        sell_price = highs[hit_idx] if hit_idx in tp_hits else lows[hit_idx]
                    else:
