...
class Backtest:
    """
    Backtesting class for trading strategies based on predicted directions.
    
    Accepts CSVs or DataFrames for minute-level price data and predictions.
    """
    ...
    def check_tp_sl(self, np_temp, np_temp_high, np_temp_low):
        ...
        if tp_sl_hit:
            pnl = ((self.sell_price - self.buy_price) / self.buy_price * 100) if self.previous_pred_direction > 0 else ((self.buy_price - self.sell_price) / self.buy_price * 100)
            pnl *= self.leverage
            pnl -= self.transaction_fee_percent + self.slippage
    ...
    def get_interval_min_data(self, index):
        start_time = np.datetime64(self.np_model_predictions[index][self.index_pred_datetime])
        end_time = np.datetime64(self.np_model_predictions[index + 1][self.index_pred_datetime])
        indices = np.where((self.np_1m[:, self.index_1m_datetime] >= start_time) &
                           (self.np_1m[:, self.index_1m_datetime] < end_time))[0]
        np_temp = self.np_1m[indices]
        np_temp_high = self.np_1m[indices, self.index_1m_high]
        np_temp_low = self.np_1m[indices, self.index_1m_low]
    ...
    def run(self):
        ...
        for i in range(len(self.np_model_predictions) - 1):
            ...
            if self.in_position and self.previous_pred_direction == self.current_pred_direction:
