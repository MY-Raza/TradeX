...
def main() -> None:
    ...
    for symbol in symbols:
        ...
        #   and a summary of how many diffs were applied per column.
        ...
        #   the actual rows seen by the trainers.
        ...
        for clf_name, is_active in classifiers_config.items():
            ...
            try:
                # Build classification target from stationary features
                ...
                try:
                    ...
                    logger.info(
                        f"[Dry-run] {clf_name} predictions on first 5 test rows:\n"
                        f"{sample_preds}"
                    )
                ...
                logger.error(f"Classifier {clf_name} failed for {symbol}: {exc}")
        ...
        for reg_name, is_active in regressors_config.items():
            ...
            try:
                # Build regression target from stationary features
                ...
                try:
                    sample_preds = model.predict(X_test.head(5))
                    logger.info(
                        f"[Dry-run] {reg_name} predictions on first 5 test rows:\n"
                        f"{sample_preds}"
                    )
                except Exception as exc:
                ...
                logger.error(f"Regressor {reg_name} failed for {symbol}: {exc}")
